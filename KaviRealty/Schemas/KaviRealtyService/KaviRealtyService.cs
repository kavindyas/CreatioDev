    ...
    public class RealtyService : BaseService, IReadOnlySessionState
    {
            ...
            string entityName)
        {
            ...
            Select select = new Select(UserConnection)
                .Column(Func.Sum("KaviPriceUSD"))
                .From(entityName)
                .Where("KaviTypeId").IsEqual(Column.Parameter(new Guid(realtyTypeId)))
                .And("KaviOfferTypeId").IsEqual(Column.Parameter(new Guid(realtyOfferTypeId)))
                as Select;
