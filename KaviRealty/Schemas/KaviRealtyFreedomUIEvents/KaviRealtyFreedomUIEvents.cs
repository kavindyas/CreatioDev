namespace Terrasoft.Configuration
{
    ...
    [EntityEventListener(SchemaName = "KaviRealty")]
    public class RealtyEntityEventListener : BaseEntityEventListener
    {
        public override void OnInserting(object sender, EntityBeforeEventArgs e)
        {
            ...
            decimal price = realty.GetTypedColumnValue<decimal>("KaviPriceUSD");
            if (price > 1000000000)
            {
                e.IsCanceled = true;

                string messageTemplate = new LocalizableString(realty.UserConnection.ResourceStorage,
                    "KaviRealtyFreedomUIEvents", "LocalizableStrings.ValueIsTooBig.Value").ToString();
